...
public protocol DaySelectorItemProtocol: AnyObject {
    ...
    func updateStyle(_ newStyle: CalendarStyle.DaySelector)
}
...
public final class DaySelector: UIView {
    ...
    private var style = CalendarStyle.DaySelector()
    ...
    public var selectedIndex = -1 {
        didSet {
            items.filter {$0.selected == true}.first?.selected = false
    ...
    // Store last selected date
        ...
        for _ in 1...daysInWeek {
            ...
            let recognizer = UITapGestureRecognizer(target: self,                                                  action: #selector(DaySelector.dateLabelDidTap(_:)))
    ...
    //      label.date = startDate.add(TimeChunk.dateComponents(days: increment), calendar: calendar)
            if let date = calendar.date(byAdding: .day, value: increment, to: startDate) {
                label.date = date
            }
        }
    }
    ...
    override public func layoutSubviews() {
        ...
        for (i, item) in items.enumerated() {
            let origin = CGPoint(x: minX + (size.width + per) * CGFloat(i), y: 0)
